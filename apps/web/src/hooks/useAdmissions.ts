/**
 * Admission Domain React Query Hooks - v1.0.0
 * 
 * Custom hooks for admission/application data fetching and mutations.
 * 
...
export const useApplicationActivity = (
  ...
  return useQuery({
    queryKey: admissionKeys.activity(id),
...
export const useUpdateApplication = (
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: admissionKeys.detail(variables.id) });
      queryClient.invalidateQueries({ queryKey: admissionKeys.lists() });
    },
    ...options,
  });
 ...
 */
export const useCompleteMyApplication = (
  options?: UseMutationOptions<ApplicationRead, Error, ApplicationUpdate>
) => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (data: ApplicationUpdate) => admissionApi.completeMyApplication(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: admissionKeys.myApplication() });
    },
    ...options,
  });
};
...
export const useVerifyDocument = (
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: admissionKeys.detail(variables.applicationId) });
    },
    ...options,
  });
};
...
export const useInitiatePayment = (
  options?: UseMutationOptions<any, Error, { id: number; amount: number }>
) => {
  return useMutation({
    mutationFn: ({ id, amount }) => admissionApi.initiatePayment(id, amount),
    ...options,
  });
};
...
export const useUpdateAdmissionSettings = (
  ...
  return useMutation({
    mutationFn: (data) => admissionApi.updateSettings(data),
    onSuccess: () => {
